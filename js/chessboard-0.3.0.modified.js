 ...
 * Date: 10 Aug 2013
 *
 * August 2018, modified by James Stanley for use in the 4x4 chess puzzle thing
 */
...
function validSquare(square) {
  ...
  return (square.search(/^[a-d][1-4]$/) !== -1);
}
...
function validFen(fen) {
  ...
  // FEN should be 4 sections separated by slashes
  ...
  for (var i = 0; i < 4; i++) {
    if (chunks[i] === '' ||
        chunks[i].length > 4 ||
        chunks[i].search(/[^kqrbnpKQRNBP1-4]/) !== -1) {
      return false;
    }
  }
...
// fudge factor, and then keep reducing until we find an exact mod 4 for
// our square size
function calculateSquareSize() {
  ...
  while (boardWidth % 4 !== 0 && boardWidth > 0) {
    boardWidth--;
  }
...
widget.resize = function() {
  ...
  // set board width
  boardEl.css('width', (SQUARE_SIZE * 4) + 'px');
